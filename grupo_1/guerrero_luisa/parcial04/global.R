...
ruta_modelo_lsa <- file.path(
  "objetos_busqueda",
  "modelo_lsa_completo.rds"
)

ruta_funciones_tfidf <- file.path(
  "objetos_busqueda",
  "funciones_busqueda_tfidf.RData"
)

ruta_funciones_lsa <- file.path(
  "objetos_busqueda",
...
archivos_buscador <- c(
  ruta_modelo_tfidf,
  ruta_modelo_lsa,
  ruta_funciones_tfidf,
  ruta_funciones_lsa
)

archivos_faltantes <- archivos_buscador[
  !file.exists(archivos_buscador)
]

if (length(archivos_faltantes) > 0) {
  stop(
    paste0(
      "Faltan archivos necesarios para el buscador: ",
      paste(
        archivos_faltantes,
        collapse = ", "
      )
...
matriz_lsa <- objeto_lsa$matriz_lsa
matriz_v <- objeto_lsa$matriz_v
valores_singulares <- objeto_lsa$valores_singulares
dimension_lsa <- objeto_lsa$dimension_lsa
porcentaje_conservado <- objeto_lsa$porcentaje_conservado
normas_documentos_lsa <- objeto_lsa$normas_documentos_lsa
memoria_tfidf_mb <- objeto_lsa$memoria_tfidf_mb
memoria_lsa_mb <- objeto_lsa$memoria_lsa_mb
reduccion_memoria <- objeto_lsa$reduccion_memoria
...
objetos_no_cargados <- objetos_requeridos[
  !vapply(
    objetos_requeridos,
    exists,
    logical(1),
...
if (length(objetos_no_cargados) > 0) {
  stop(
    paste0(
      "No se cargaron correctamente estos objetos: ",
      paste(
        objetos_no_cargados,
        collapse = ", "
      )
...
message(
  "Buscador TF-IDF y LSA cargado correctamente."
)
