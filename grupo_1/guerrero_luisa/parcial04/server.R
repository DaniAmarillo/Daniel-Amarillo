...
server <- function(input, output, session) {
  ...
  resultados_buscador <- reactiveVal(data.frame())
  comparacion_buscador <- reactiveVal(data.frame())
  consulta_buscador_activa <- reactiveVal("")
  estrategia_buscador_activa <- reactiveVal("")
  mensaje_buscador_activo <- reactiveVal(
    "Escriba una consulta y presione 'Buscar artículos'."
  )
  
  preparar_resultados_buscador <- function(datos) {
    ...
    datos %>%
      mutate(
        ...
        publication_date = as.character(
          publication_date
        ),
        estrategia = case_when(
          str_detect(estrategia, "TF-IDF") ~ "TF-IDF",
          str_detect(estrategia, "LSA") ~ "LSA",
          TRUE ~ estrategia
        ),
        enlace_doi = ifelse(
          is.na(enlace_doi) | enlace_doi == "",
          "",
          paste0(
            "<a href='",
            enlace_doi,
        ...
        detalle = paste0(
          "<button type='button' ",
          "class='btn btn-default btn-sm detalle-articulo' ",
          "data-paper='",
          paper_id,
          "'>",
          "Ver detalle",
  ...
  observeEvent(input$ejecutar_busqueda, {
    consulta <- str_squish(input$consulta_buscador)
    estrategia <- input$estrategia_buscador
    n_resultados <- as.integer(input$n_resultados_buscador)
    
    if (is.null(consulta) || consulta == "") {
      resultados_buscador(data.frame())
      comparacion_buscador(data.frame())
      consulta_buscador_activa("")
      estrategia_buscador_activa("")
      mensaje_buscador_activo(
        "Debe escribir una consulta antes de ejecutar la búsqueda."
      )
      
      showNotification(
        "Escriba una consulta para buscar artículos.",
        type = "warning",
        duration = 4
      )
    ...
    mensaje_buscador_activo("Procesando la consulta…")
    
    tryCatch(
      {
        if (estrategia == "TF-IDF") {
          resultados <- buscar_articulos_tfidf(
            consulta = consulta,
            n_resultados = n_resultados
          )
          
          resultados_buscador(resultados)
          comparacion_buscador(data.frame())
          estrategia_mostrada <- "TF-IDF"
        } else if (estrategia == "LSA") {
          resultados <- buscar_articulos_lsa(
            consulta = consulta,
            n_resultados = n_resultados
          )
          ...
          resultados_buscador(resultados)
          comparacion_buscador(data.frame())
          estrategia_mostrada <- "LSA"
        } else {
          resultados_tfidf <- buscar_articulos_tfidf(
            consulta = consulta,
            n_resultados = n_resultados
          )
          
          resultados_lsa <- buscar_articulos_lsa(
            consulta = consulta,
            n_resultados = n_resultados
          )
          ...
          comparacion <- full_join(
            ...
            mutate(
              aparece_tfidf = ifelse(
                is.na(posicion_tfidf),
                "No",
                "Sí"
              ),
              aparece_lsa = ifelse(
            ...
            arrange(
              desc(coincidencia == "Ambas"),
              posicion_tfidf,
              posicion_lsa
            )
          
          resultados_buscador(resultados)
          comparacion_buscador(comparacion)
          estrategia_mostrada <- "Ambas"
        }
        
        consulta_buscador_activa(consulta)
        estrategia_buscador_activa(estrategia_mostrada)
        
        mensaje_buscador_activo(
          paste0(
            "Búsqueda completada. Se recuperaron ",
            nrow(resultados),
            " filas para la consulta seleccionada."
          )
        )
      },
      error = function(e) {
        ...
        comparacion_buscador(data.frame())
        consulta_buscador_activa(consulta)
        estrategia_buscador_activa(estrategia)
  ...
  observeEvent(input$limpiar_busqueda, {
    updateTextAreaInput(
      session,
      "consulta_buscador",
      value = ""
  ...
  output$buscador_consulta_activa <- renderText({
    consulta <- consulta_buscador_activa()
  ...
  output$buscador_estrategia_activa <- renderText({
    estrategia <- estrategia_buscador_activa()
  ...
  output$buscador_total_resultados <- renderText({
    datos <- resultados_buscador()
    estrategia <- estrategia_buscador_activa()
    ...
    if (estrategia == "Ambas") {
      cantidad_por_metodo <- nrow(datos) / 2
  ...
  output$mensaje_buscador <- renderUI({
    mensaje <- mensaje_buscador_activo()
    
    if (str_detect(mensaje, "No fue posible")) {
      tags$p(
        style = "color: #b00020; font-weight: 700;",
        mensaje
      )
    } else {
      tags$p(mensaje)
  ...
  output$tabla_resultados_buscador <- renderDT({
    datos <- preparar_resultados_buscador(
      resultados_buscador()
    )
    
    if (nrow(datos) == 0) {
      datos <- data.frame(
        Mensaje = "Aún no se han ejecutado búsquedas."
      )
    }
    ...
    datatable(
      ...
      callback = JS(
        "
        table.on('click', 'button.detalle-articulo', function() {
  ...
  observeEvent(input$ver_detalle_articulo, {
    datos <- resultados_buscador()
    
    req(
      nrow(datos) > 0,
      input$ver_detalle_articulo
    )
    
    articulo <- datos %>%
      filter(
        as.integer(paper_id) ==
          as.integer(input$ver_detalle_articulo)
      ) %>%
      slice(1)
    
    req(nrow(articulo) == 1)
    
    autores <- articulo$authors[1]
    fragmento <- articulo$fragmento[1]
    enlace <- articulo$enlace_doi[1]
    ...
    if (
      is.na(fragmento) ||
      str_squish(fragmento) == ""
    ) {
      fragmento <- "No hay resumen o fragmento disponible para este artículo."
    }
    ...
    showModal(
      modalDialog(
        title = tags$div(
          style = "font-weight: 800; line-height: 1.25;",
          articulo$title[1]
        ),
        ...
        div(
          ...
          h4(
            style = "margin-top: 4px; font-weight: 800;",
            "Autores"
          ),
          ...
          h4(
            style = "margin-top: 20px; font-weight: 800;",
            "Resumen o fragmento"
          ),
  ...
  output$tabla_comparacion_buscador <- renderDT({
    datos <- comparacion_buscador()
    
    if (nrow(datos) == 0) {
      datos <- data.frame(
        Mensaje = "La comparación aparecerá al seleccionar ambas estrategias."
      )
    } else {
      datos <- datos %>%
        transmute(
          "ID" = paper_id,
          "Título" = title,
          "Posición TF-IDF" = posicion_tfidf,
          "Puntaje TF-IDF" = round(
            as.numeric(puntaje_tfidf),
            4
          ),
          "Posición LSA" = posicion_lsa,
          "Puntaje LSA" = round(
            as.numeric(puntaje_lsa),
            4
          ),
  ...
  # Scraping: buscar nuevos artículos: buscar nuevos artículos
  # -------------------------------------------------------
