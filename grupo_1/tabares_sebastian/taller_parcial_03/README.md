...
El documento principal construye una variable objetivo binaria a nivel de
beneficiario, entrena y compara cinco modelos de clasificacion supervisada
(Arbol de decision, Random Forest, XGBoost, SVM, KNN), aplica dos tecnicas de
analisis no supervisado (K-means, Isolation Forest), estima el costo asociado
a la atencion de beneficiarios con TEA y ajusta un modelo de costo esperado
(GLM Gamma) que combina simultaneamente TEA, edad, hospitalizaciones e
ingresos a UTI para distintos perfiles de beneficiario.

NOTA SOBRE EL NOMBRE DEL ARCHIVO: este README fue escrito usando el nombre
"taller_3_Tabares-Segovia.Rmd" para el documento principal. Si tu archivo real
en el repositorio se llama distinto (por ejemplo
"taller-parcial_3_Tabares-Segovia.Rmd", como aparece en tu carpeta local),
renombra uno de los dos para que coincidan, y ajusta la referencia en este
README y en cualquier script que lo invoque por nombre.

--------------------------------------------------------------------------------
2. INVENTARIO DE ARCHIVOS DE LA CARPETA PERSONAL
--------------------------------------------------------------------------------

    carpeta_personal/
    |-- taller_3_Tabares-Segovia.Rmd     <- documento fuente (codigo + texto)
    |-- taller_3_Tabares-Segovia.html    <- salida generada con "Knit to HTML"
    |-- taller_3_Tabares-Segovia.pdf     <- salida generada con "Knit to PDF"
    |-- README.txt                       <- este archivo
    |-- data/
    |   `-- db_2026.csv                  <- base de datos original (no versionada, ~1.6 GB)
    |
    |   (objetos intermedios generados automaticamente por el propio .Rmd
    |    al ejecutarse, mediante saveRDS(); no se crean a mano)
    |-- variables_utilizacion.rds        <- variables de utilizacion/costos agregadas (seccion 4.2.3)
    |-- base_beneficiarios_curada.rds    <- base analitica final, ya limpia (seccion 5.7.2.2)
    |-- modelo_cart.rds                  <- arbol de decision entrenado (seccion 7.1)
    |-- modelo_rf.rds                    <- Random Forest entrenado (seccion 7.2)
    |-- modelo_xgb.rds                   <- XGBoost entrenado (seccion 7.3)
    |-- train_escalado.rds               <- particion de entrenamiento estandarizada (seccion 8.1.1)
    |-- test_escalado.rds                <- particion de prueba estandarizada (seccion 8.1.1)
    |-- modelo_svm.rds                   <- SVM entrenado (seccion 8.2)
    |-- modelo_knn.rds                   <- KNN entrenado (seccion 8.3)
    |-- datos_cluster.rds                <- muestra usada para K-means/Isolation Forest (seccion 8.4)
    `-- modelo_costo.rds                 <- modelo GLM Gamma de costo esperado (seccion 10.6)

De estos, taller_3_Tabares-Segovia.html y taller_3_Tabares-Segovia.pdf son las
UNICAS dos salidas que se generan manualmente desde RStudio (boton "Knit" /
"Knit to PDF"); todo lo demas (los .rds) se genera solo al ejecutar el .Rmd de
principio a fin. db_2026.csv debe copiarse manualmente; no se incluye en el
...
--------------------------------------------------------------------------------
  ...
  - Paquetes de R utilizados a lo largo del documento:
    ...
    glm() con family = Gamma(link = "log") (seccion 10.6) NO requiere ningun
    paquete adicional: forma parte de "stats", incluido en la instalacion
    base de R.

  - Solo si se quiere generar el PDF (ademas del HTML): una distribucion
    LaTeX. La forma mas simple es instalar tinytex una sola vez:

      install.packages("tinytex")
      tinytex::install_tinytex()

    Sin esto, el boton "Knit to PDF" fallara. El HTML (formato principal
    exigido por el enunciado) no requiere LaTeX ni tinytex.
...
--------------------------------------------------------------------------------
  ...
  4. Verificar que el working directory sea la carpeta personal (donde esta
     ...
     esa carpeta.
  5. Para generar el HTML: boton "Knit" (o "Knit to HTML" si el desplegable
     lo muestra). Para generar el PDF: desplegable junto al boton "Knit" ->
     "Knit to PDF" (requiere tinytex, ver seccion 3). El documento vuelve a
     leer la base completa, recalcula la limpieza y las variables, reentrena
     los cinco modelos supervisados, las dos tecnicas no supervisadas y el
     modelo de costo esperado, y genera el reporte en el formato elegido.
  ...
  puede tardar varios minutos, tanto para HTML como para PDF (ambos formatos
  vuelven a ejecutar todo el codigo).
...
la opcion number_sections del YAML se dejo en false para AMBOS formatos de
salida (html_document y pdf_document), para que Pandoc no agregue una
numeracion automatica adicional que chocaria con la numeracion manual. La
jerarquia de secciones de la seccion 7 de este README debe coincidir
exactamente con los encabezados del documento renderizado, tanto en HTML
como en PDF.

--------------------------------------------------------------------------------
6. SALIDA EN PDF: NOTAS TECNICAS
--------------------------------------------------------------------------------

  - code_folding y toc_float (YAML) son opciones exclusivas de html_document;
    R Markdown las ignora automaticamente al usar "Knit to PDF" (no generan
    error, simplemente no aplican en PDF).
  - Se configuro latex_engine: xelatex en lugar del motor por defecto
    (pdflatex), porque maneja mejor los acentos y la enie del espanol.
  - Algunas figuras del documento se generan con fig.width/fig.height grandes
    (por ejemplo, el arbol de decision en la seccion 7.1 usa fig.width=18),
    pensadas para pantalla. En PDF pueden verse mas comprimidas que en HTML;
    si el arbol se ve ilegible en el PDF, se puede reducir fig.width en ese
    chunk especifico solo para la version PDF.

--------------------------------------------------------------------------------
7. JERARQUIA COMPLETA DE SECCIONES DEL DOCUMENTO
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
8. ACLARACION: DE 6 ETAPAS A 11 SECCIONES
--------------------------------------------------------------------------------
...
Sin embargo, como se ve en la jerarquia de la seccion 7 de este README, el
...
--------------------------------------------------------------------------------
9. HALLAZGOS RELEVANTES DE CALIDAD DE DATOS (seccion 5.3 del documento)
--------------------------------------------------------------------------------

El analisis de las secciones 5.3.1 y 5.3.2 encontro un problema de calidad de
datos que NO habia sido detectado por la revision inicial de valores
faltantes (seccion 3.2), porque se trata de valores de texto ("-" o
categorias vacias) y no de NA tecnicos:

  - UF_CNES_PREST_HOSPITALAR (estado): 28.75% de las utilizaciones sin
    informacion valida.
  - DESC_ESPECIALIDADE (especialidad): ~73% de las utilizaciones sin
    informacion valida en toda la base, y 99.88% sin informacion valida
    especificamente entre las utilizaciones con diagnostico de TEA.

Esto ultimo es relevante porque especialidades_distintas (construida a
partir de DESC_ESPECIALIDADE) es una variable predictora importante tanto en
los modelos de clasificacion (seccion 9.2) como en el modelo de costo
esperado (seccion 10.6): con un 99.88% de datos faltantes entre los casos de
TEA, no puede descartarse que parte de su importancia refleje un patron de
registro diferencial y no solo diversidad clinica real. Ver seccion 11.1 del
documento para la discusion completa.
...
--------------------------------------------------------------------------------
  ...
  - La estimacion de costos (seccion 10.6) usa un modelo Gamma con enlace
    ...
    alternativas (regresion log-lineal, modelos de dos partes, etc.). La
    tabla de coeficientes tampoco incluye errores estandar ni valores p;
    para confirmar significancia estadistica hay que revisar la salida
    completa de summary(modelo_costo).
  - Variables como el numero de especialidades consultadas y los costos
    ...
    11.1 del documento, junto con el hallazgo de la seccion 9 de este README.
