...
function(input, output, session) {
  ...
  # Tarjeta HTML para un solo resultado
  tarjeta_resultado <- function(fila) {
    fecha <- if (is.na(fila$publication_date) || fila$publication_date == "") "s.f." else fila$publication_date
    resumen <- fila$abstract
    if (is.na(resumen) || nchar(resumen) == 0) resumen <- "(sin resumen disponible)"
    resumen <- paste0(substr(resumen, 1, 220), ifelse(nchar(resumen) > 220, "...", ""))

    div(style = "background:#f9faf8; border:1px solid #e2ebd9; border-radius:8px; padding:12px 15px; margin-bottom:10px;",
        div(style = "display:flex; justify-content:space-between;",
            span(style = "font-weight:700; color:#1b4332;", paste0("#", fila$posicion, " — ", fila$title)),
            span(style = "font-weight:700; color:#2d6a4f;", paste0("score: ", fila$score))
        ),
        div(style = "font-size:12px; color:#555; margin-top:4px;",
            paste0(fila$authors_raw, " · ", fecha, " · ", fila$topic_label)),
        div(style = "font-size:13px; margin-top:6px;", resumen),
        div(style = "font-size:12px; margin-top:6px;",
            tags$a(href = fila$doi, target = "_blank", fila$doi))
    )
  }

  bloque_estrategia <- function(nombre, res) {
    if (!res$ok) {
      return(div(class = "alert alert-warning", res$msg))
    }
    tagList(
      h5(style = "font-weight:700; color:#2d6a4f;", nombre),
      p(style = "font-size:11px; color:#888;",
        paste0("Términos usados de la consulta: ", paste(unique(res$tokens_usados), collapse = ", "))),
      lapply(seq_len(nrow(res$results)), function(i) tarjeta_resultado(res$results[i, ]))
    )
  }

  observeEvent(input$btn_buscar, {
    query <- input$query_busqueda
    n     <- as.integer(input$n_resultados)

    if (is.null(query) || nchar(trimws(query)) == 0) {
      output$resultados_busqueda <- renderUI(
        div(class = "alert alert-warning", "Escribe una consulta antes de buscar.")
      )
      return()
    }

    res <- buscar(query, search_index, metodo = input$metodo_busqueda, n = n)

    output$resultados_busqueda <- renderUI({
      if (input$metodo_busqueda == "ambas") {
        fluidRow(
          column(6, bloque_estrategia("BM25 (léxico, dimensión original)", res$bm25)),
          column(6, bloque_estrategia(paste0("LSA (k = ", search_index$k_lsa, " dimensiones)"), res$lsa))
        )
      } else if (input$metodo_busqueda == "bm25") {
        bloque_estrategia("BM25 (léxico, dimensión original)", res$bm25)
      } else {
        bloque_estrategia(paste0("LSA (k = ", search_index$k_lsa, " dimensiones)"), res$lsa)
      }
    })
  })
