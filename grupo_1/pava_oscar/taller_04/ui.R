  ...
  fluidRow(
    ...
    column(9,
           ...
           tabsetPanel(
             ...
             tabPanel("\U0001f50e Buscador de Artículos",
                      br(),
                      div(class = "section-box",
                          h4(style="font-weight:700; color:#1b4332; margin-top:0;",
                             "Recuperación de Información (BM25 vs. LSA / Truncated SVD)"),
                          fluidRow(
                            column(8,
                                   textAreaInput("query_busqueda",
                                                 "Escribe tu consulta en lenguaje natural (en inglés, idioma del corpus):",
                                                 placeholder = "Ej. generative AI applications in disease diagnosis",
                                                 width = "100%", rows = 2)
                            ),
                            column(2,
                                   selectInput("n_resultados", "Nº de resultados:",
                                               choices = c(5, 10, 20), selected = 10)
                            ),
                            column(2,
                                   radioButtons("metodo_busqueda", "Estrategia:",
                                                choices = c("BM25 (léxico)" = "bm25",
                                                            "LSA reducido (semántico)" = "lsa",
                                                            "Comparar ambas" = "ambas"),
                                                selected = "ambas")
                            )
                          ),
                          actionButton("btn_buscar", "\U0001f50d Buscar",
                                       class = "btn btn-block btn-success",
                                       style = "background-color:#2d6a4f; border:none; font-weight:600; max-width:220px;"),
                          p(style="font-size:11px; color:#888; margin-top:8px;",
                            paste0("El vocabulario y las representaciones vectoriales se calcularon una sola vez ",
                                   "(precompute_search.R) y se cargan desde search_index.rds; no se reentrena nada en cada búsqueda.")),
                          hr(),
