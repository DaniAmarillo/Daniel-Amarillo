...
.bus-controles { display: flex; flex-wrap: wrap; gap: 28px; margin-top: 14px; }
.bus-control label { display: block; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; color: rgba(255,255,255,0.40); margin-bottom: 6px; }
.bus-control .shiny-options-group { display: flex; flex-wrap: wrap; gap: 14px; }
.bus-control .radio-inline { color: rgba(255,255,255,0.72); font-size: 12px; font-weight: 500; margin: 0; }
.bus-ejemplos { margin-top: 16px; padding-top: 14px; border-top: 1px solid rgba(255,255,255,0.06); display: flex; flex-wrap: wrap; gap: 10px; align-items: center; }
.bus-ejemplos > span { font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; color: rgba(255,255,255,0.30); }
.bus-ejemplos a { font-size: 11px; color: #a5b4fc; background: rgba(99,102,241,0.10); border: 1px solid rgba(99,102,241,0.20); border-radius: 20px; padding: 4px 12px; text-decoration: none; cursor: pointer; }
.bus-ejemplos a:hover { background: rgba(99,102,241,0.22); color: #c7d2fe; }
.hit { display: flex; gap: 16px; padding: 18px 0; border-bottom: 1px solid rgba(255,255,255,0.05); }
.hit:last-child { border-bottom: none; }
.hit-rank { flex: 0 0 40px; font-family: var(--font-mono); font-size: 22px; font-weight: 700; color: rgba(255,255,255,0.18); line-height: 1; padding-top: 2px; }
.hit-body { flex: 1; min-width: 0; }
.hit-title { font-size: 14px; font-weight: 700; color: var(--text-primary); line-height: 1.4; margin-bottom: 6px; }
.hit-meta { font-size: 11px; color: rgba(255,255,255,0.42); margin-bottom: 8px; line-height: 1.6; }
.hit-meta .sep { color: rgba(255,255,255,0.18); margin: 0 7px; }
.hit-frag { font-size: 12px; color: rgba(255,255,255,0.58); line-height: 1.65; font-style: italic; border-left: 2px solid rgba(99,102,241,0.28); padding-left: 11px; margin-bottom: 9px; }
.hit-foot { display: flex; flex-wrap: wrap; gap: 14px; align-items: center; font-size: 11px; }
.hit-doi { font-family: var(--font-mono); color: #818cf8; text-decoration: none; }
.hit-doi:hover { color: #c7d2fe; text-decoration: underline; }
.hit-tag { background: rgba(255,255,255,0.05); border-radius: 5px; padding: 2px 9px; color: rgba(255,255,255,0.55); }
.hit-score { flex: 0 0 92px; text-align: right; }
.hit-score b { display: block; font-family: var(--font-mono); font-size: 15px; color: #22d3ee; }
.hit-score span { font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; color: rgba(255,255,255,0.30); }
.hit-bar { height: 3px; background: rgba(255,255,255,0.06); border-radius: 2px; margin-top: 6px; overflow: hidden; }
.hit-bar > div { height: 100%; background: linear-gradient(90deg, #6366f1, #22d3ee); }
.bus-cols { display: flex; gap: 20px; align-items: flex-start; }
.bus-cols > div { flex: 1; min-width: 0; }
.bus-aviso { background: rgba(245,158,11,0.06); border: 1px solid rgba(245,158,11,0.22); border-radius: var(--radius-lg); padding: 18px 20px; color: rgba(255,255,255,0.62); font-size: 12px; line-height: 1.6; }
.doi-result { background: rgba(99,102,241,0.05); border: 1px solid rgba(99,102,241,0.22); border-radius: var(--radius-lg); padding: 18px 20px; margin-bottom: 22px; }
.doi-result.miss  { background: rgba(245,158,11,0.05); border-color: rgba(245,158,11,0.25); }
.doi-result.error { background: rgba(244,63,94,0.05);  border-color: rgba(244,63,94,0.25); }
.doi-result h4 { color: var(--text-primary); font-size: 15px; font-weight: 700; margin: 6px 0 10px 0; line-height: 1.4; }
.doi-code { font-family: var(--font-mono); font-size: 12px; color: #a5b4fc; background: rgba(255,255,255,0.04); padding: 3px 9px; border-radius: 6px; }
.doi-meta { display: flex; flex-wrap: wrap; gap: 18px; margin-top: 14px; }
.doi-meta > div { font-size: 10px; text-transform: uppercase; letter-spacing: 0.4px; color: rgba(255,255,255,0.40); }
.doi-meta strong { display: block; font-size: 18px; color: var(--text-primary); font-family: var(--font-mono); font-weight: 700; margin-bottom: 2px; }
.doi-btn { display: inline-block; background: rgba(99,102,241,0.18); color: #a5b4fc; border: 1px solid rgba(99,102,241,0.30); border-radius: var(--radius-sm); padding: 8px 16px; font-size: 12px; font-weight: 600; text-decoration: none; cursor: pointer; transition: 0.2s; }
.doi-btn:hover { background: rgba(99,102,241,0.28); color: #c7d2fe; }
.doi-btn.warn { background: rgba(245,158,11,0.15); color: #fbbf24; border-color: rgba(245,158,11,0.30); }
.doi-btn.warn:hover { background: rgba(245,158,11,0.25); }
.sync-chip { flex: 1; min-width: 84px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; padding: 10px 12px; text-align: center; }
.sync-chip b { display: block; font-size: 20px; font-weight: 700; font-family: var(--font-mono); }
.sync-chip span { font-size: 10px; text-transform: uppercase; letter-spacing: 0.4px; color: rgba(255,255,255,0.40); }
    "))
  ),
  ...
  div(id = "app-shell",
      ...
      tags$nav(id = "sidebar",
               ...
               tags$details(open = TRUE, class = "sidebar-accordion",
                            ...
                            div(class = "accordion-content",
                                ...
                                actionButton("nav_buscador", class = "nav-item-btn", onclick = "switchPage('buscador')", label = tagList(tags$span(class = "nav-icon", tags$i(class = "fa-solid fa-magnifying-glass")), "Buscador")),
               ...
               tags$details(open = TRUE, class = "sidebar-accordion",
                            ...
                            div(class = "accordion-content",
                                ...
                                div(class = "filter-group", tags$span(class = "filter-label", "DOI"),
                                    textInput("busqueda_doi", NULL, placeholder = "10.1007/s10462-...", width = "100%"),
                                    div(style = "display:flex; gap:6px; margin-top:6px;",
                                        actionButton("btn_buscar_doi", class = "doi-btn", style = "flex:1; padding:7px; font-size:11px;",
                                                     label = tagList(tags$i(class = "fa-solid fa-fingerprint"), " Buscar")),
                                        actionButton("btn_limpiar_doi", class = "doi-btn", style = "padding:7px 11px; font-size:11px; background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.5); border-color:rgba(255,255,255,0.10);",
                                                     label = tags$i(class = "fa-solid fa-xmark"))
                                    ),
                                    tags$span(style = "display:block; margin-top:6px; font-size:10px; color:rgba(255,255,255,0.35); line-height:1.4;",
                                              "DOI puro, doi:10.1007/... o la URL de SpringerLink."),
                                    tags$script(HTML("$(document).on('keypress', '#busqueda_doi', function(e){ if(e.which == 13){ $('#btn_buscar_doi').click(); } });"))
                                ),
      ...
      div(id = "main-content",
          ...
          # PAGE 2: ANALYTICS
          # PAGE 1B: BUSCADOR (Taller 4)
          div(id = "page-buscador", class = "page-content page-section", style = "display:none",
              div(class = "page-header",
                  tags$h1("Buscador de Art\u00edculos"),
                  tags$p("Recuperaci\u00f3n por consulta en lenguaje natural sobre t\u00edtulo, resumen y palabras clave")),

              div(class = "glass-card",
                  div(class = "card-header",
                      div(class = "card-header-icon", tags$i(class = "fa-solid fa-magnifying-glass")),
                      tags$h3("Consulta"),
                      tags$span(class = "card-sub", uiOutput("bus_estado_indice", inline = TRUE))
                  ),
                  div(class = "card-body",
                      textAreaInput("bus_consulta", NULL, width = "100%", rows = 2,
                                    placeholder = "Ej: aplicaciones de inteligencia artificial generativa en el diagn\u00f3stico de enfermedades"),

                      div(class = "bus-controles",
                          div(class = "bus-control",
                              tags$label("Estrategia de recuperaci\u00f3n"),
                              radioButtons("bus_estrategia", NULL, inline = TRUE,
                                           choices = c("BM25 (l\u00e9xica)"                  = "bm25",
                                                       "TF-IDF (sin reducir)"                = "tfidf",
                                                       "LSA (reducida)"                      = "lsa",
                                                       "H\u00edbrida (RRF)"                  = "hibrido",
                                                       "Comparar BM25 vs LSA"                = "comparar",
                                                       "Ablaci\u00f3n: TF-IDF vs LSA"        = "ablacion"),
                                           selected = "hibrido")
                          ),
                          div(class = "bus-control",
                              tags$label("Resultados"),
                              radioButtons("bus_top_n", NULL, inline = TRUE,
                      ...
                      div(style = "display:flex; gap:10px; align-items:center; margin-top:14px;",
                          actionButton("bus_buscar", class = "doi-btn",
                                       label = tagList(tags$i(class = "fa-solid fa-magnifying-glass"), " Buscar")),
                          actionButton("bus_limpiar", class = "doi-btn",
                                       style = "background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.5); border-color:rgba(255,255,255,0.10);",
                                       label = tagList(tags$i(class = "fa-solid fa-xmark"), " Limpiar")),
                          tags$span(style = "font-size:11px; color:rgba(255,255,255,0.35);",
                                    uiOutput("bus_diagnostico", inline = TRUE))
                      ),

                      div(class = "bus-ejemplos",
                          tags$span("Consultas de ejemplo:"),
                          actionLink("bus_ej1", "generative AI for medical diagnosis"),
                          actionLink("bus_ej2", "explainable machine learning"),
                          actionLink("bus_ej3", "aprendizaje profundo en im\u00e1genes m\u00e9dicas"),
                          actionLink("bus_ej4", "swarm optimization algorithms"),
                          actionLink("bus_ej5", "privacidad y seguridad en federated learning")
                      ),
                      tags$script(HTML("$(document).on('keydown', '#bus_consulta', function(e){ if(e.which == 13 && !e.shiftKey){ e.preventDefault(); $('#bus_buscar').click(); } });"))
                  )
              ),

              uiOutput("bus_resultados")
          ),
  ...
  # ============================================================
  tags$script(HTML("
    var pages = ['dashboard', 'buscador', 'analytics', 'literature', 'authors', 'insights', 'admin'];
