...
library(purrr)
library(highcharter)
library(Matrix)       # buscador: matrices dispersas
library(text2vec)     # buscador: vectorización de la consulta
library(SnowballC)    # buscador: stemming

DB_PATH <- "nature_eco_evo_2025.sqlite"

# ════════════════════════════════════════════════════════════
# BUSCADOR — índice precalculado + funciones de recuperación
# El índice se construye una sola vez con build_index.R y se
# carga aquí; NO se recalcula en cada búsqueda.
# ════════════════════════════════════════════════════════════
source("search_engine.R", local = TRUE)
IDX <- if (file.exists("search_index.rds")) readRDS("search_index.rds") else NULL
...
ui <- navbarPage(
  ...
  tabPanel("Buscador",
    sidebarLayout(
      sidebarPanel(width = 3,
        tags$h5("Búsqueda semántica", class="text-primary fw-bold"), tags$hr(),
        textAreaInput("q_texto", "Consulta en lenguaje natural:",
                      placeholder = "ej. generative AI for disease diagnosis",
                      rows = 3, resize = "vertical"),
        radioButtons("q_estrategia", "Estrategia de recuperación:",
          choices = c(
            "Léxica — TF-IDF + coseno"          = "lexico",
            "Semántica — LSA (SVD) + coseno"    = "semantico",
            "Comparar ambas"                     = "ambas"),
          selected = "ambas"),
        selectInput("q_topk", "N.º de resultados:",
                    choices = c(5, 10, 20), selected = 10),
        actionButton("q_buscar", "Buscar",
                     icon = icon("magnifying-glass"), class="btn-primary w-100"),
        tags$hr(),
        helpText("La consulta se proyecta al mismo espacio vectorial de los",
                 "artículos (TF-IDF o LSA) y se ordena por similitud coseno.")
      ),
      mainPanel(width = 9,
        tags$div(class="alert alert-light border-start border-primary border-3 p-2 mb-3",
          style="font-size:0.85rem;",
          tags$strong("Recuperación de información. "),
          "Se comparan dos estrategias: recuperación ",
          tags$strong("léxica"), " (TF-IDF disperso + similitud coseno) y ",
          tags$strong("semántica reducida"),
          " (LSA / Truncated SVD sobre el TF-IDF + coseno). ",
          "El puntaje es la similitud coseno en [0, 1]; ",
          "no es una probabilidad y no es comparable entre estrategias distintas."
        ),
        uiOutput("q_info_indice"),
        uiOutput("q_resultados")
      )
    )
  ),
...
server <- function(input, output, session) {
    ...
    showModal(modalDialog(
      ...
      fluidRow(
        column(6,
          ...
          tags$p(icon("magnifying-glass"), tags$strong(" Buscador:"), " recuperación semántica de artículos")
        ),
        column(6,
  ...
  output$q_info_indice <- renderUI({
    if (is.null(IDX)) {
      return(tags$div(class="alert alert-warning",
        icon("triangle-exclamation"),
        tags$strong(" Índice no encontrado. "),
        "Ejecute ", tags$code("Rscript build_index.R"),
        " para generar ", tags$code("search_index.rds"), "."))
    }
    tags$p(class="text-muted", style="font-size:0.8rem;",
      icon("database"),
      sprintf(" Índice: %d artículos · vocabulario %d términos · LSA k=%d (%.0f%% de varianza).",
              IDX$n_docs, IDX$n_terms, IDX$k, 100 * IDX$var_expl_k))
  })

  # Tarjeta de un resultado individual
  tarjeta_resultado <- function(r) {
    enlace <- if (!is.na(r$url)) r$url
              else if (!is.na(r$doi)) paste0("https://doi.org/", r$doi)
              else "#"
    tags$div(class="card mb-2 shadow-sm border-0",
      style="border-left:4px solid #8CBF3C !important;",
      tags$div(class="card-body p-2",
        tags$div(class="d-flex justify-content-between align-items-start",
          tags$h6(class="mb-1",
            tags$span(class="badge bg-primary me-2", paste0("#", r$ranking)),
            tags$a(href=enlace, target="_blank",
                   style="text-decoration:none;color:#1a1a1a;", r$title)),
          tags$span(class="badge bg-success", sprintf("%.3f", r$score))
        ),
        tags$p(class="mb-1 text-muted", style="font-size:0.78rem;",
          icon("user-group"),
          " ", ifelse(is.na(r$authors_raw), "—", substr(r$authors_raw, 1, 90)),
          tags$br(),
          icon("calendar"), " ", r$publication_date, "  ·  ",
          icon("tag"), " ", r$topic_label, "  ·  ",
          icon("quote-right"), " ", ifelse(is.na(r$citations), 0, r$citations), " citas  ·  ",
          if (!is.na(r$doi))
            tags$a(href=paste0("https://doi.org/", r$doi), target="_blank",
                   paste0("DOI: ", r$doi))
          else "sin DOI"
        ),
        tags$p(class="mb-0", style="font-size:0.8rem;", r$fragmento)
      )
    )
  }

  bloque_resultados <- function(res, titulo) {
    if (is.null(res) || nrow(res) == 0)
      return(tags$p(class="text-muted", "Sin resultados."))
    tagList(
      tags$h6(class="text-primary fw-bold mt-2", titulo),
      lapply(seq_len(nrow(res)), function(i) tarjeta_resultado(res[i, ]))
    )
  }

  resultados_busqueda <- eventReactive(input$q_buscar, {
    req(IDX)
    q  <- input$q_texto
    tk <- as.integer(input$q_topk)
    validate(need(nchar(trimws(q)) > 0, "Escriba una consulta para buscar."))
    list(
      lexico    = buscar(q, IDX, "lexico",    tk),
      semantico = buscar(q, IDX, "semantico", tk),
      estrategia = input$q_estrategia
    )
  })

  output$q_resultados <- renderUI({
    R <- resultados_busqueda()
    if (R$estrategia == "lexico")
      bloque_resultados(R$lexico, "Resultados — Recuperación léxica (TF-IDF + coseno)")
    else if (R$estrategia == "semantico")
      bloque_resultados(R$semantico, "Resultados — Recuperación semántica (LSA + coseno)")
    else
      fluidRow(
        column(6, bloque_resultados(R$lexico,    "Léxica (TF-IDF)")),
        column(6, bloque_resultados(R$semantico, "Semántica (LSA)"))
      )
  })
