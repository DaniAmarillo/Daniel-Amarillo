...
.result-title a:hover { color: var(--accent); text-decoration: underline; }
.result-meta {
  font-size: 12px;
  color: var(--muted);
  margin-bottom: 8px;
...
.result-strategy-col { padding: 0 8px; }

/* ── Resultados en grilla flexible (se adapta al número de estrategias) ── */
.results-grid {
  display: grid;
...
/* ── Resaltado de términos coincidentes en la búsqueda ── */
mark.hl {
...
/* ── Animaciones: la app se siente más viva ── */
@keyframes fadeInUp {
...
.scrape-box { animation: fadeInUp 0.35s ease both; }
.search-box { transition: box-shadow 0.25s ease, border-color 0.25s ease; }
.search-box:focus-within { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(59,130,246,0.15); }

.btn-scrape { transition: opacity 0.2s ease, transform 0.15s ease, box-shadow 0.2s ease !important; }
.btn-scrape:hover { transform: translateY(-2px); box-shadow: 0 8px 18px rgba(59, 130, 246, 0.35) !important; }
.btn-scrape:active { transform: translateY(0); }

/* Barra de progreso superior mientras Shiny procesa (búsqueda, filtros, scraping) */
...
ui <- fluidPage(
  tags$head(
    ...
    tags$title("PLOS ONE ·  Buscador de artículos")
  ),
  
  
  div(class = "app-header",
      div(class = "app-title", "PLOS", tags$span(" ONE"), " · Buscador de artículos")
  ),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        # ── Tab 4: Buscador (Taller 4) ───────────────────────────────────────
        tabPanel("🔎 Buscador",
                 br(),
                 div(class = "search-box",
                     textAreaInput("query_busq",
                                   label = "Escribe tu consulta",
                                   placeholder = "Ej: aplicaciones de inteligencia artificial generativa en el diagnóstico de enfermedades",
                                   width = "100%", rows = 2
                     ),
                     div(class = "hint-text", style = "margin-top:-4px;",
                         "🌐 El corpus está en inglés — si escribes en español, la consulta se traduce automáticamente con un diccionario básico antes de buscar."
                     ),
                     fluidRow(
                       column(4,
                              selectInput("estrategia_busq", "Estrategia de recuperación",
                                          choices = c(
                                            "TF-IDF (léxica)"       = "tfidf",
                                            "BM25 (léxica)"         = "bm25",
                                            "LSA (semántica)"       = "lsa",
                                            "Híbrida (TF-IDF+LSA)"  = "hibrida",
                                            "Comparar todas"        = "todas"
                                          ),
                                          selected = "tfidf"
                              )
                       ),
                       ...
                       column(4, style = "display:flex; align-items:flex-end;",
                              actionButton("btn_buscar", "🔎 Buscar", class = "btn-scrape")
                       )
                     )
                 ),
                 br(),
                 div(class = "hint-text",
                     "💡 Haz clic en el título de cualquier artículo para abrirlo en la página oficial"
                 ),
