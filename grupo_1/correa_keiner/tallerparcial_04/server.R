...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_scrape, {
    ...
    if (resultado$nuevos == 0) {
      rv$scrape_msg  <- paste0(
        "ℹ️ No se encontraron artículos nuevos. Se re-scrapearon en vivo y se actualizaron ",
        length(resultado$verificados), " artículo(s) (vistas, citas, referencias):\n",
        paste(resultado$verificados, collapse = "\n")
      )
      rv$scrape_tipo <- "warn"
      rv$df_nuevos   <- NULL
      rv$n_nuevos    <- 0
      # Refrescar datos filtrados por si los valores actualizados afectan la vista actual
      rv$df_filtrado <- get_papers_filtrados()
    } else {
  ...
  output$chart_temporal <- renderHighchart({
    ...
    df_mes <- df %>%
      dplyr::mutate(mes = format(as.Date(publication_date), "%Y-%m")) %>%
      count(mes) %>%
      dplyr::arrange(mes)
  ...
  output$chart_top_autores <- renderHighchart({
    ...
    df_autores <- df %>%
      ...
      dplyr::mutate(author = strsplit(authors, ",\\s*")) %>%
      unnest(author) %>%
  ...
  output$tabla_papers <- renderDT({
    ...
    df %>%
      dplyr::mutate(
        DOI_link = sprintf('<a href="%s" target="_blank" rel="noopener" style="color:var(--accent); text-decoration:underline;">%s</a>', url, doi)
      ) %>%
      dplyr::select(
        Título        = title,
        Autores       = authors,
  ...
  # Nombres legibles para cada código de estrategia 
  .nombres_estrategia <- c(
    tfidf   = "TF-IDF + coseno (léxica)",
    bm25    = "BM25 (léxica)",
    lsa     = "LSA + coseno (semántica)",
    hibrida = "Híbrida (TF-IDF + LSA)"
  )
  
  observeEvent(input$btn_buscar, {
    q <- trimws(input$query_busq)
    n <- as.integer(input$n_resultados)
    seleccion <- input$estrategia_busq
    
    if (q == "") {
      rv$resultados <- list()
      showNotification("Escribe una consulta antes de buscar.", type = "warning")
      return()
    }
    
    estrategias_sel <- if (seleccion == "todas") names(.nombres_estrategia) else seleccion
    
    resultados <- lapply(estrategias_sel, function(cod) buscar_articulos(q, cod, n))
    names(resultados) <- estrategias_sel
    rv$resultados <- resultados
  })
  
  # Construye el HTML de una tarjeta de resultado individual.

  .render_result_card <- function(fila) {
    div(class = "result-card",
        style = sprintf("animation-delay: %.2fs;", min((fila$posicion - 1) * 0.04, 0.6)),
        span(class = "result-score", sprintf("%.3f", fila$score)),
        span(class = "result-rank", paste0("#", fila$posicion)),
        div(class = "result-title",
            tags$a(href = fila$url, target = "_blank", rel = "noopener",
                   title = "Clic para abrir el artículo original",
                   HTML(fila$title_hl))
        ),
        div(class = "result-meta",
            fila$authors, " · ", fila$publication_date, " · ", fila$topic_label, " · DOI: ",
            tags$a(href = fila$url, target = "_blank", rel = "noopener", fila$doi)
        ),
        div(class = "result-snippet", HTML(paste0(fila$fragmento_hl, "…")))
    )
  }
  
  .render_result_list <- function(df) {
    if (is.null(df)) {
      return(div(class = "scrape-box warn",
                 " Esta estrategia no está disponible todavía: falta el archivo de índice correspondiente. Ejecuta build_search_index.R para generarlo y reinicia la app."))
    }
    if (nrow(df) == 0) {
      return(div(class = "scrape-box warn", "No se encontraron artículos para esta consulta (ningún término coincide con el vocabulario del corpus)."))
    }
    tagList(lapply(seq_len(nrow(df)), function(i) .render_result_card(df[i, ])))
  }
  
  # La grilla se adapta sola al número de estrategias elegidas (1 a 4):
  output$resultados_busqueda <- renderUI({
    if (length(rv$resultados) == 0) {
      return(div(style = "color: var(--muted); font-family: 'Inter', sans-serif; font-size:13px;",
                 "Escribe una consulta, elige una o más estrategias y presiona 🔎 Buscar."))
    }
    
    div(class = "results-grid",
        lapply(names(rv$resultados), function(cod) {
          div(class = "result-strategy-col",
              h4(.nombres_estrategia[[cod]],
                 style = "color: var(--accent); font-family:'Outfit',sans-serif; font-size:14px;"),
              .render_result_list(rv$resultados[[cod]])
          )
        })
    )
  })
