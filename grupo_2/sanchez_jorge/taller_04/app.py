...
# Buscador de artículos (Taller 4)
@st.cache_resource
def cargar_buscador():
    """Carga una sola vez los objetos precalculados del buscador.

    Gracias a @st.cache_resource la matriz TF-IDF, el modelo LSA y el índice
    BM25 se construyen (o se leen del .joblib) una única vez por sesión: la
    app nunca revectoriza el corpus en cada búsqueda.
    """
    return bus.cargar_artefactos()

def render_buscador_tab():
    st.markdown('<div class="section-title">Búsqueda de artículos científicos</div>',
                unsafe_allow_html=True)

    try:
        artefactos = cargar_buscador()
    except Exception as e:
        st.error(f"No se pudo inicializar el buscador: {e}")
        return

    meta = artefactos["meta"]
    st.markdown(
        f'<div class="info-box">Índice sobre <b>{meta["n_documentos"]}</b> artículos · '
        f'TF-IDF de <b>{meta["dim_original"]}</b> términos · '
        f'espacio latente LSA de <b>{meta["dim_reducida"]}</b> componentes '
        f'({meta["varianza_acumulada"]:.0%} de varianza retenida).</div>',
        unsafe_allow_html=True)

    consulta = st.text_input(
        "Consulta en lenguaje natural",
        placeholder="Ej. monetary policy transmission and bank lending",
        key="bus_consulta")

    c1, c2, c3 = st.columns([2.4, 1.1, 1.1])
    etiquetas = {
        "A": "A · TF-IDF + coseno (léxica)",
        "B": "B · LSA + coseno (semántica latente, reducida)",
        "C": "C · BM25 Okapi (léxica)",
        "AB": "Comparar A vs B",
        "ABC": "Comparar las tres",
    }
    modo = c1.selectbox("Estrategia de recuperación", list(etiquetas.keys()),
                        format_func=lambda k: etiquetas[k], key="bus_modo")
    top_k = c2.selectbox("Resultados", [5, 10, 20], index=1, key="bus_topk")
    buscar_btn = c3.button("Buscar", type="primary", use_container_width=True)

    if not consulta.strip():
        st.markdown('<div class="warning-box">Escribe una consulta para iniciar la búsqueda.</div>',
                    unsafe_allow_html=True)
        return
    if not buscar_btn and not st.session_state.get("bus_ejecutada"):
        st.session_state["bus_ejecutada"] = True

    estrategias = list(modo) if modo in ("AB", "ABC") else [modo]

    if len(estrategias) == 1:
        _mostrar_resultados(consulta, artefactos, estrategias[0], top_k)
    else:
        cols = st.columns(len(estrategias))
        for col, est in zip(cols, estrategias):
            with col:
                _mostrar_resultados(consulta, artefactos, est, top_k, compacto=True)

def _mostrar_resultados(consulta, artefactos, estrategia, top_k, compacto=False):
    res, ms = bus.buscar_con_tiempo(consulta, artefactos, estrategia, top_k)
    st.markdown(f"**{bus.ESTRATEGIAS[estrategia]}** · {ms:.0f} ms")

    if res.empty or res["puntaje"].max() <= 0:
        st.markdown('<div class="warning-box">Ningún artículo coincide con la consulta.</div>',
                    unsafe_allow_html=True)
        return
    ...
    if compacto:
        tabla = res[["posicion", "title", "puntaje"]].copy()
        tabla.columns = ["#", "Título", "Puntaje"]
        tabla["Puntaje"] = tabla["Puntaje"].round(3)
        st.dataframe(tabla, use_container_width=True, hide_index=True, height=380)
        return

    for _, r in res.iterrows():
        fecha = str(r["publication_date"])[:10] or "—"
        enlace = r["url"] or (f"https://doi.org/{r['doi']}" if r["doi"] else "")
        doi_html = (f'<a href="{enlace}" target="_blank">{r["doi"] or enlace}</a>'
                    if enlace else "—")
        st.markdown(f"""
        <div style="background:#ffffff;border:1px solid #e2e8f0;border-left:4px solid #2563eb;
          ...
          <div style="display:flex;justify-content:space-between;gap:1rem;">
            ...
            <div style="white-space:nowrap;color:#1e40af;font-weight:700;font-size:.95rem;">
              {r['puntaje']:.4f}
            </div>
          </div>
          <div style="color:#64748b;font-size:.78rem;margin:.35rem 0;">
            {r['authors_raw'] or '—'} · {fecha} · {r['topic_label'] or '—'} · {doi_html}
          </div>
          <div style="color:#475569;font-size:.83rem;line-height:1.45;">{r['fragmento']}</div>
        </div>""", unsafe_allow_html=True)
...
def main():
    ...
    <div style="background:linear-gradient(135deg,#1e3a8a 0%,#2563eb 100%);
      ...
      <p style="margin:.4rem 0 0;font-size:.9rem;opacity:.85;">
        Minería de Datos · Talleres 2 y 4 · Jorge Andres Sanchez Duarte · Universidad Nacional de Colombia
      </p>
    </div>""", unsafe_allow_html=True)
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
        [" Resumen", " Visualizaciones", " Buscador", " Datos", " Scraping", " Gestión"])
    ...
    with tab3: render_buscador_tab()
    with tab4: render_tabla(df)
    with tab5: render_scraping_tab()
    with tab6: render_crud_tab(df)

    st.markdown("---")
    st.markdown("<div style='text-align:center;color:#94a3b8;font-size:.78rem;'>"
                "Minería de Datos · Talleres 2 y 4 · 2026 · Universidad Nacional de Colombia</div>",
                unsafe_allow_html=True)
