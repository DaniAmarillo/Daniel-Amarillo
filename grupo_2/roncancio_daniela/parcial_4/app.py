...
from buscador import Buscador, entrenar_y_guardar
...
tab_info, tab_dash, tab_cluster, tab_buscar = st.tabs([
    "📖 Información",
    "📈 Dashboard",
    "🧩 Clustering",
    "🔍 Buscador",
...
with tab_cluster:
    ...
    st.markdown(
        ...
        "- **Cluster 0 — Artículos estándar (196 artículos):** representa la gran "
        "mayoría de la producción de la revista. Tiene pocas citas (0.31 en "
        "promedio), visibilidad moderada (1170 vistas) y más autores por artículo "
        "(4.09). Son artículos recientes o de nicho que aún no han acumulado "
...
# ===========================================================================
with tab_buscar:
    st.subheader("🔍 Buscador de artículos científicos")
    st.markdown(
        "Escribe una consulta en **lenguaje natural** y el sistema recuperará "
        "los artículos más relevantes de la base de datos usando técnicas de "
        "procesamiento de texto y representación vectorial."
    )

    # --- Inicializar buscador con caché de Streamlit ---
    @st.cache_resource(show_spinner="Cargando modelos de búsqueda...")
    def cargar_buscador(db_path: str):
        """Carga el buscador UNA SOLA VEZ. Si los modelos no existen, los entrena."""
        from buscador import Buscador, entrenar_y_guardar
        import os
        if not os.path.exists("modelos/tfidf_vectorizer.pkl"):
            entrenar_y_guardar(db_path)
        return Buscador(db_path)

    try:
        buscador = cargar_buscador(DB_PATH)
        modelos_ok = True
    except Exception as e:
    ...
    if modelos_ok:
        ...
        col_q, col_m, col_n = st.columns([3, 1, 1])
        with col_q:
            consulta = st.text_input(
                "💬 Consulta en lenguaje natural",
                placeholder="Ej: deep learning for medical image classification",
            )
        with col_m:
            metodo = st.selectbox(
                "Estrategia",
                options=["tfidf", "lsa", "ambos"],
                format_func=lambda x: {
                    "tfidf": "TF-IDF (léxica)",
                    "lsa":   "LSA (semántica)",
                    "ambos": "Ambas estrategias",
                }[x],
            )
        with col_n:
            top_n = st.selectbox("Resultados", options=[5, 10, 20], index=1)

        buscar_btn = st.button("🔍 Buscar", type="primary")

        # --- Información sobre las estrategias ---
        with st.expander("ℹ️ ¿Qué diferencia hay entre las estrategias?"):
            st.markdown(
                "**TF-IDF + Similitud Coseno (léxica):** busca artículos que "
                "comparten los mismos términos que la consulta. Es precisa cuando "
                "usas exactamente las palabras del artículo.\n\n"
                "**LSA + Similitud Coseno (semántica):** reduce las dimensiones con "
                "Truncated SVD y compara en un espacio latente. Captura relaciones "
                "semánticas entre palabras (sinónimos, conceptos relacionados) aunque "
                "no aparezcan exactamente en el texto.\n\n"
                f"Dimensión TF-IDF: **{buscador.tfidf_matrix.shape[1]} términos** → "
                f"LSA reducida a **{buscador.lsa_matrix.shape[1]} componentes** "
                f"(40,2% varianza explicada)."
            )

        # --- Ejecución de la búsqueda ---
        if buscar_btn and consulta.strip():
            with st.spinner("Buscando artículos relevantes..."):
                resultados = buscador.buscar(consulta, metodo=metodo, top_n=top_n)

            if resultados.empty:
                st.warning("No se encontraron resultados. Intenta con otros términos.")
            else:
                st.success(
                    f"Se encontraron resultados para: *\"{consulta}\"* "
                    f"| Estrategia: **{metodo.upper()}** | Top {top_n}"
                )

                # Mostrar resultados por estrategia
                for met in resultados["Método"].unique():
                    sub = resultados[resultados["Método"] == met].reset_index(drop=True)
                    if metodo == "ambos":
                        st.markdown(f"### Resultados — {met}")
                    ...
                    for _, row in sub.iterrows():
                        with st.container():
                            ...
                            with c1:
                                ...
                                st.markdown(
                                    f"**{row['Posición']}. [{row['Título']}]({doi_url})**"
                                )
                                st.caption(
                                    f"👤 {row['Autores'][:80]}  |  📅 {row['Fecha']}  "
                                    f"|  🏷️ {row['Tema']}"
                                )
                                st.markdown(f"_{row['Fragmento']}_")
                            with c2:
                                st.metric("Relevancia", f"{row['Puntaje']:.3f}")
                            st.markdown("---")
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"resultados_{metodo}.csv",
                    mime="text/csv",
                )

        elif buscar_btn and not consulta.strip():
        ...
        with st.expander("⚙️ Opciones avanzadas"):
            st.caption(
                "Si actualizaste la base de datos con nuevos artículos, "
                "reentrena los modelos para incluirlos en las búsquedas."
            )
            if st.button("🔄 Reentrenar modelos de búsqueda"):
                with st.spinner("Entrenando TF-IDF y LSA..."):
                    try:
                        metricas = entrenar_y_guardar(DB_PATH)
                        cargar_buscador.clear()
                        st.success(
                            f"✅ Modelos reentrenados. "
                            f"Corpus: {metricas['n_incluidos']} artículos | "
                            f"Vocabulario: {metricas['vocabulario']} términos | "
                            f"LSA: {metricas['dim_reducida']} componentes "
                            f"({metricas['varianza_explicada_lsa']*100:.1f}% varianza)"
                        )
                    except Exception as e:
                        st.error(f"Error al entrenar: {e}")

st.caption("Minería de Datos · Streamlit + SQLite · Taller 2 + Taller 4")
