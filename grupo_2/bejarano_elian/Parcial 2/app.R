...
server <- function(input, output, session) {
  ...
  output$tabla_calidad <- renderDT({
    tbl <- calidad_df() |>
      ...
      select(paper_id, Título=title, Fecha, Temática=topic_label, Citas=citations, Descargas=downloads, Vistas=views, Referencias=n_references, Motivo)
  ...
  output$tabla_nuevos <- renderDT({
    ...
    tbl <- df |>
      ...
      select(Título,Autores=authors_raw,Fecha=publication_date,
             Temática,DOI=DOI_link,Citas=citations,Descargas=downloads)
