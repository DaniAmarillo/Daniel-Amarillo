...
library(xml2)
library(rsconnect)
library(Matrix)          # Taller 4: matrices dispersas

DB_PATH <- "nature_srep_2025.db"

# ── Taller 4: buscador ────────────────────────────────────────
#  Funciones generadas por taller_4.Rmd. La cache se carga UNA sola
#  vez al arrancar: durante una busqueda no se toca SQLite ni se
#  reconstruye ninguna matriz (§4.7 del enunciado).
source("R/funciones_buscador.R")
IND_0  <- readRDS("modelos/indices.rds")
META_0 <- readRDS("modelos/metadatos.rds")
...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      ...
      # ── TAB 2: Buscador (Taller 4) ───────────────────────
      tabItem("buscador",
        buscadorUI("buscador", k_lsa = IND_0$lsa$k)
      ),
      ...
      tabItem("update",
        fluidRow(
          box(title = "Scraping de nuevos artículos", width = 12, status = "primary",
              ...
              tags$hr(),
              tags$p(style = "color:#8b949e; font-size:13px;",
                "Tras agregar artículos, el índice del buscador queda desactualizado. ",
                "Reconstrúyalo para que los nuevos artículos sean recuperables."),
              actionButton("reindex_btn", "Reconstruir índice del buscador",
                           class = "btn-primary", icon = icon("rotate")),
              verbatimTextOutput("reindex_log")
          )
        )
      )
    )
...
server <- function(input, output, session) {
  ...
  # ── Taller 4: buscador ──────────────────────────────────────
  #  Los indices van en reactiveVal para poder reconstruirlos tras un
  #  scraping sin reiniciar la aplicacion.
  ind_rv  <- reactiveVal(IND_0)
  meta_rv <- reactiveVal(META_0)
  buscadorServer("buscador",
                 indices   = reactive(ind_rv()),
                 metadatos = reactive(meta_rv()))

  reindex_log <- reactiveVal("")
  observeEvent(input$reindex_btn, {
    withProgress(message = "Reconstruyendo indice...", value = 0.3, {
      res <- tryCatch(construir_indices(DB_PATH), error = function(e) e)
      if (inherits(res, "error")) {
        reindex_log(paste("✗ Error:", conditionMessage(res)))
        return(invisible(NULL))
      }
      incProgress(0.6)
      ind_rv(res$indices); meta_rv(res$metadatos)
      dir.create("modelos", showWarnings = FALSE)
      saveRDS(res$indices,   "modelos/indices.rds")
      saveRDS(res$metadatos, "modelos/metadatos.rds")
      reindex_log(sprintf(
        "✓ Indice reconstruido.\n  Articulos: %d\n  Vocabulario: %d terminos\n  Componentes LSA (k): %d",
        nrow(res$metadatos), length(res$indices$vocab), res$indices$lsa$k))
    })
  })
  output$reindex_log <- renderText(reindex_log())
